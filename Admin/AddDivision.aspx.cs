...
namespace LMS.Admin
{
    ...
    {
        DivisionBLL bll = new DivisionBLL();
        int deletingID = 0;
        ...
        private void BindGrid(string p)
        {
            ...
            dt = bll.GetDivisionList("Full");
            DataView DataView = dt.DefaultView;
            DataView.RowFilter = "Section LIKE '%" + search + "%'";
        ...
        protected void Button9_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Division name cannot be empty')", true);
                }
                else
                {
                    ...
                    dtdepartmentstandard = bll.GetDivisionList("Full");
                    List<string> departmentstandard = new List<string>();
                    departmentstandard = dtdepartmentstandard.AsEnumerable()
                   .Select(r => r.Field<string>("Section"))
                   .ToList();
                    if (!departmentstandard.Contains(txtDivisionName.Text))
                    {
                        var result = bll.AddNewDivision(txtDivisionName.Text, isPublished);
                        if (result > 0)
                        {
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Division Registered Sucessfully')", true);
                            getclients();
                        }
                        else
                        {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Division cannot be empty')", true);
                }
                else
                {
                    Boolean isPublished = false;
                    ...
                    var result = bll.UpdateDivision(int.Parse(lblID.Text), txtname.Text, isPublished);
                    if (result > 0)
                    {
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Division Updated Sucessfully')", true);
                        getclients();
                    }
                    else
                    {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var result = bll.DeleteDivision(departmentDeleteId);
                if (result > 0)
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Division Deleted Sucessfully')", true);
                    getclients();
                }
                else
                {
        ...
        protected void btnPublish_Click(object sender, EventArgs e)
        {
            try
            {
                List<DivisionList> examAnswerList = new List<DivisionList>();
                foreach (GridViewRow row in GridView1.Rows)
                {
                    Label labelid = row.FindControl("lblid") as Label;
                    ...
                    examAnswerList.Add(new DivisionList { Id = id, IsActive = isActive });
                }
                int result = bll.PublishDivision(examAnswerList);
                if (result > 0)
                {
                    getclients();
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Division Published Successfully')", true);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Something went wrong!')", true);
        ...
        private void ExportGridView()
        {
            ...
            string attachment = "attachment; filename=DivisionList.xls";
            ...
            string headerTable = @"<Table><tr><td colspan='5' align='center'><font size='7'>Division List</font></td></tr>
